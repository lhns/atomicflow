...
lazy val root = (project in file("."))
  ...
  .aggregate(core, upickleModule, db)
...
lazy val upickleModule = (project in file("upickle"))
  .dependsOn(core % "compile->compile;test->test")
  .settings(
    name := "atomicflow-upickle",
    libraryDependencies ++= Seq(
      "ch.qos.logback" % "logback-classic" % V.logbackClassic % Test,
...
lazy val db = project
  .dependsOn(core % "compile->compile;test->test", upickleModule % Test)
  .settings(
    name := "atomicflow-db",
    libraryDependencies ++= Seq(
